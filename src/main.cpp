...
void drawClusters( cv::Mat &img, std::vector<std::vector<double> > &lines, std::vector<std::vector<int> > &clusters )
{
    ...
    lineColors[0] = cv::Scalar( 0, 255, 255 );
...
int main()
{
    string inPutImage = "/home/npcdna/VanishingPointDetection/d_12.jpg";
    ...
    LineDetect( image, thLength, lines );//只提取30像素以上的线条
    std::cout<<"线条长度："<<lines.size()<<std::endl;
	// Camera internal parameters
	cv::Point2d pp( 318, 193 );	
	//cv::Point2d pp( 307, 251 );        // Principle point (in pixel)
    double f = 200;//焦距单位不确定暂时用200
